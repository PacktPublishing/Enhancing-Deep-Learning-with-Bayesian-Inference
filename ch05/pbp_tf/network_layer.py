...
class NetworkLayer:
    def __init__(self, m_w_init, v_w_init, non_linear=True, layer_index: Optional[int] = None):
    ...
    @tf.function
    def n_pdf(x):
        return 1. / tf.math.sqrt(2 * tf.constant(math.pi, dtype=tf.float64)) * tf.math.exp(0.5 * x ** 2)

    @staticmethod
    @tf.function
    def n_cdf(x):
        return 0.5 * (1.0 + tf.math.erf(x / tf.math.sqrt(tf.constant(2.0, dtype=tf.float64))))
    ...
    def output_probabilistic(self, m_w_previous, v_w_previous):
        ...
        m_w_previous_with_bias = tf.cast(tf.concat([m_w_previous, tf.Variable([1], dtype=tf.float64)], 0), tf.float64)
        v_w_previous_with_bias = tf.cast(tf.concat([v_w_previous, tf.Variable([0], dtype=tf.float64)], 0), tf.float64)
        # We compute the mean and variance after the linear operation
        # import ipdb
        # ipdb.set_trace()

        m_linear = tf.tensordot(
            self.m_w, m_w_previous_with_bias, 1) / tf.cast(tf.math.sqrt(self.n_inputs), dtype=tf.float64)
        v_linear = (
        ...
        if self.non_linear:
            ...
            gamma_final = tf.where(-alpha > tf.cast(tf.fill(alpha.shape, 30.), tf.float64), tf.cast(gamma, tf.float64), tf.cast(gamma_robust, tf.float64))
    ...
    def output_deterministic(self, output_previous):
        ...
        output_previous_with_bias = tf.concat(
            [output_previous, tf.constant([1., 1.])], 0
