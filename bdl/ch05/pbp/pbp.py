...
from bdl.ch05.pbp import network

from bdl.ch05.pbp import prior
...
class PBP:
    def __init__(self, layer_sizes, mean_y_train, std_y_train):
        ...
        self.x = np.ndarray([])
        self.y = 0
        # self.x = tf.Variable(initial_value=0, name="x")
        # self.y = tf.Variable(initial_value=1, name="y")
    ...
    def do_first_pass(self, X, y):
        ...
        for i in permutation:
            ...
            # TODO:: FIX ADF UPDATE
            # logZ = self.network.generate_updates(X[i, :], y[i], self.logZ, self.logZ1, self.logZ2)
