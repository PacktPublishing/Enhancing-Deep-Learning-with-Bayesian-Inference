...
class Network_layer(tf.keras.layers.Layer):
    def __init__(self, m_w_init, v_w_init, non_linear=True, layer_index: int = None):
        super().__init__(dtype=tf.as_dtype(tf.float32))
        ...
        self.m_w = self.add_weight(
            shape=m_w_init.shape,
            name="m_w", dtype=tf.float32,
        ...
        self.v_w = self.add_weight(
            shape=v_w_init.shape,
            name="v_w", dtype=tf.float32
        )

        self.w = self.add_weight(name="w", dtype=tf.float32, shape=m_w_init.shape)
    ...
    def output_probabilistic(self, m_w_previous, v_w_previous):
        ...
        m_w_previous_with_bias = m_w_previous
        v_w_previous_with_bias = v_w_previous
        # We compute the mean and variance after the linear operation

        m_linear = tf.tensordot(self.m_w, tf.cast(m_w_previous_with_bias, tf.float32), axes=0) / tf.sqrt(self.n_inputs)
        v_linear = (
                           tf.tensordot(self.v_w, tf.cast(v_w_previous_with_bias, tf.float32), axes=0)
                           + tf.tensordot(self.m_w**2, tf.cast(v_w_previous_with_bias, tf.float32), axes=0)
                           + tf.tensordot(self.v_w, tf.cast(m_w_previous_with_bias, tf.float32)**2, axes=0)
                   ) / self.n_inputs
