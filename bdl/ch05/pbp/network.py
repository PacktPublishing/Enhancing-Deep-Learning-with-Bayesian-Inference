...
from bdl.ch05.pbp import network_layer
...
class Network:
    def __init__(self, m_w_init, v_w_init, a_init, b_init):
        ...
        if len(m_w_init) > 1:
            for idx, (m_w, v_w) in enumerate(zip(m_w_init[:-1], v_w_init[:-1])):
                self.layers.append(
                    network_layer.Network_layer(m_w, v_w, True, layer_index=idx)
                )

        self.layers.append(
            network_layer.Network_layer(m_w_init[-1], v_w_init[-1], False, idx + 1)
        )
    ...
    @tf.function
    def generate_updates(self, X, y, logZ, logZ1, logZ2):
        ...
        for i in range(len(self.params_m_w)):
            updates.append(
                (
                    self.params_m_w[i],
                    self.params_m_w[i]
                    + self.params_v_w[i] * tf.gradients(logZ, self.params_m_w[i]),
                )
            )
            updates.append(
                (
                    ...
                    - self.params_v_w[i] ** 2
                    * (
                        tf.gradients(logZ, self.params_m_w[i]) ** 2
                        - 2 * tf.gradients(logZ, self.params_v_w[i])
                    ),
                )
            )
        ...
        updates.append(
            (
                self.a,
                1.0 / (tf.math.exp(logZ2 - 2 * logZ1 + logZ) * (self.a + 1) / self.a - 1.0),
            )
        )
        updates.append(
            (
                ...
                1.0
                / (
                    tf.math.exp(logZ2 - logZ1) * (self.a + 1) / (self.b)
                    - tf.math.exp(logZ1 - logZ) * self.a / self.b
                ),
            )
        )
    ...
    def set_params(self, params):
        ...
        for i in range(len(self.layers)):
            self.layers[i].m_w = params["m_w"][i]
            self.layers[i].v_w = params["v_w"][i]
