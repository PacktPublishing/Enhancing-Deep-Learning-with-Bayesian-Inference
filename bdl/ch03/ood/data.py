...
def get_test_data() -> Tuple[tf.data.Dataset, pd.DataFrame]:
    ...
    df_test["path"] = df_test["path"].apply(lambda x: str(DATA_ROOT / f"oxford-iiit-pet/images/{x}.jpg"))
...
def get_train_val_data() -> Tuple[tf.data.Dataset, tf.data.Dataset]:
    df = read_data()
    paths_train, paths_val, labels_train, labels_val = train_test_split(df["path"], df["breed"], test_size=0.2, random_state=0)
    train_dataset = (
        tf.data.Dataset.from_tensor_slices((paths_train, labels_train)).map(lambda x, y: preprocess(x, y))
        .batch(256)
